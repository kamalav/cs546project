...
public class FeatureQuantizer {

	public static FeatureVector[] prototypeVectors;
	...
	private static FeatureVector[] trainPrototypeVectorsByKMeans(int k,
		...
		for (int i = 0; i < iterations; i++) {
			...
			for (int vector = 0; vector < count; vector++) {
				...
				// if v is a centroid currently, move on
				for (int j = 0; j < k; j++) {
					FeatureVector centroid = centroids[j];
					if (centroid.equals(v)) {
						isCentroid = true;
						break;
					}
				}
	...
	static private int findNearestVectorIndex(FeatureVector vector,
		...
		for (int i = 0; i < prototypeVectors.length; i++) {
			FeatureVector v = prototypeVectors[i];
			double distance = euclideanDistance(vector, v);
			if (distance < minDistance) {
				index = i;
				minDistance = distance;
			}
	...
	static public int quantize(FeatureVector vector) {
		...
		if (Double.isNaN(vector.writingAngle))
